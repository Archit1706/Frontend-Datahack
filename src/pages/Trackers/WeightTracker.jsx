...
const WeightTracker = () => {
  ...
  const submitHandler = (e) => {
    ...
    fetch("https://backend.sidd065.repl.co/api/meals/add",{
			method: "POST",
			headers: {
      ...
      .then(res => res.json())
			.then(res=>{
				console.log(res)
				localStorage.setItem('foodCal', res.calories+ Number(localStorage.getItem("foodCal")))
        localStorage.setItem('foodPro', res.proteins+ Number(localStorage.getItem("foodPro")))
        localStorage.setItem('foodFat', res.fats+ Number(localStorage.getItem("foodFat")))
